...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
    ...
    if (border && border_size > 0.0f)
    {
        ...
        window->DrawList->AddRect(p_min+ImVec2(1, 1), p_max-ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow), rounding, ImDrawCornerFlags_All, border_size); // custom
...
void ImGui::RenderCheckMark(ImVec2 pos, ImU32 col, float sz)
{
    ...
    window->DrawList->AddRectFilled(ImVec2(pos.x - 1, pos.y - 1), ImVec2(pos.x + sz + 1, pos.y + sz + 1), GetColorU32(ImVec4(0, 0, 0, 1)), 0.0f, style.FrameRounding); // custom
    window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y), ImVec2(pos.x + sz, pos.y + sz), col, 0.0f, style.FrameRounding); // custom
    /*
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (alpha != 1.0f)
             ...
             window->DrawList->AddRect(window->Pos + ImVec2(0, window->TitleBarHeight()) + ImVec2(1, 1), window->Pos + window->Size - ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow), window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot); // custom (inset shadow)
        }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        */
        ...
        //custom
        window->InnerClipRect.Min.x = ImFloor(window->InnerRect.Min.x + ImMax(ImFloor(0), window->WindowBorderSize + 1));
        window->InnerClipRect.Min.y = ImFloor(window->InnerRect.Min.y + top_border_size + 1);
        window->InnerClipRect.Max.x = ImFloor(window->InnerRect.Max.x - ImMax(ImFloor(0), window->WindowBorderSize + 1));
        window->InnerClipRect.Max.y = ImFloor(window->InnerRect.Max.y - window->WindowBorderSize + 1);
