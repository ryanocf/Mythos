...
void ImGui::Separator()
{
    ...
    flags &= ~ImGuiSeparatorFlags_SpanAllColumns; // custom
    SeparatorEx(flags);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    flags += ImGuiComboFlags_NoArrowButton; // custom
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(w, label_size.y * 2.0f + style.FramePadding.y * 2.0f)); // custom
    // const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f + label_size.y)); // custom
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (label_size.x > 0)
        ...
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - label_size.y - style.FramePadding.y + 2), label); // custom
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float slider_usable_sz = slider_sz; // custom
    //const float slider_usable_pos_min = bb.Min[axis] + grab_padding + grab_sz * 0.5f;
    const float slider_usable_pos_min = bb.Min[axis] + grab_padding; // custom
    //const float slider_usable_pos_max = bb.Max[axis] - grab_padding - grab_sz * 0.5f;
    const float slider_usable_pos_max = bb.Max[axis] - grab_padding; // custom
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    //const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f));
    //const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(0, label_size.y - 5) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f)); // custom
    const ImRect total_bb(frame_bb.Min - ImVec2(0, label_size.y), frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f)); // custom
    ...
    */
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 2), ImVec2(grab_bb.Max.x - ((grab_bb.Max.x - grab_bb.Min.x) / 2), grab_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.FrameRounding); // custom
    }
    ...
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
    RenderTextClipped(frame_bb.Min - ImVec2(0, 17), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(1.0f,0.0f)); // custom

    if (label_size.x > 0.0f) {
        //RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 20 + style.FramePadding.y), label); // custom
    }
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            // custom
            // xi - 1 so that the line doesn't do some anti alising shit on different resolutions.
            // better the line clips to the side inside custom_imgui->CustomBorder instead of popping out on the top/bottom, what looks ugly
            // maybe still looks ugly with ImGui Border alpha < 1.0f
            window->DrawList->AddLine(ImVec2(xi - 1, y1), ImVec2(xi, y1), GetColorU32(ImGuiCol_Border));
            window->DrawList->AddLine(ImVec2(xi, y1), ImVec2(xi, y2 - 11.0f), col);
            window->DrawList->AddLine(ImVec2(xi - 1, y2 - 10.0f), ImVec2(xi, y2 - 10.0f), GetColorU32(ImGuiCol_Border));
        }
